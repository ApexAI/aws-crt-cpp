...
static bool s_parse_proxy_uri(app_ctx &ctx, const char *proxy_arg) {
    ...
    auto proxyOptions =
        Io::Socks5ProxyOptions::CreateFromUri(parsed_uri, 10000, ctx.allocator);
    if (!proxyOptions) {
 ...
 * It is intended as a minimal end-to-end test of connect, subscribe, publish, receive, and disconnect using various connection options.
 */
...
void PrintAppOptions(const app_ctx &ctx) {
    ...
    if (ctx.use_proxy && ctx.socks5_proxy_options && !ctx.proxy_host_storage.empty()) {
        ...
        const aws_socks5_proxy_options *rawProxyOptions =
            ctx.socks5_proxy_options->GetUnderlyingHandle();
...
int main(int argc, char **argv)
{
    ...
    {
        std::cout << "**********************************************************" << std::endl;
        std::cout << "MQTT5: Configuring SOCKS5 Proxy with host "
                  << app_ctx.proxy_host_storage << " and port " << app_ctx.proxy_port << std::endl;
        bool resolveViaProxy = app_ctx.socks5_proxy_options->GetHostResolutionMode() ==
                               Io::AwsSocks5HostResolutionMode::Proxy;
        std::cout << "MQTT5: SOCKS5 DNS mode: "
                  << (resolveViaProxy ? "proxy-resolved" : "client-resolved") << std::endl;

        const aws_socks5_proxy_options *rawProxyOptions =
            app_ctx.socks5_proxy_options->GetUnderlyingHandle();
        if (rawProxyOptions->username && rawProxyOptions->password)
        {
            std::cout << "MQTT5: Configuring SOCKS5 Proxy with username "
                      << aws_string_c_str(rawProxyOptions->username) << " and password ***" << std::endl;
        }
        else
        {
