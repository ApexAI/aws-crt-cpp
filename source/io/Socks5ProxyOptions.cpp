...
Socks5ProxyOptions::Socks5ProxyOptions() noexcept : m_allocator(aws_default_allocator()), m_lastError(AWS_ERROR_SUCCESS) {
...
bool Socks5ProxyOptions::SetProxyEndpoint(const String &hostName, uint32_t port) {
    ...
    const bool shouldCarryAuth =
        m_authMethod == AwsSocks5AuthMethod::UsernamePassword && m_options.username && m_options.username->len > 0 &&
        m_options.password && m_options.password->len > 0;
    ...
    aws_byte_cursor hostCursor = aws_byte_cursor_from_array(
        reinterpret_cast<const uint8_t *>(hostName.data()), hostName.length());
    ...
    if (shouldCarryAuth) {
        if (aws_socks5_proxy_options_set_auth(&newOptions, allocator, previousUsernameCursor, previousPasswordCursor)) {
            m_lastError = aws_last_error();
            aws_socks5_proxy_options_clean_up(&newOptions);
            return false;
        }
